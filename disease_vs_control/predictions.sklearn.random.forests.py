"""
Random forest predictions for each train/test split
...
for split in os.listdir("splits"):
    ...
    os.mkdir(os.path.join("predictions", "random_forest", split))
    open(os.path.join("predictions", "random_forest", split, "train_predictions_binary.tsv"), "w").write("\n".join(train_predictions.astype(np.str)))
    open(os.path.join("predictions", "random_forest", split, "test_predictions_binary.tsv"), "w").write("\n".join(test_predictions.astype(np.str)))
